...
After writing all files, spawn a subagent in the foreground (`model: "opus"`, `run_in_background: false`; omitting it backgrounds the agent) to review the skill. The subagent should read [references/skill-reviewer.md](references/skill-reviewer.md) for review guidelines, read all skill files, and produce a review report following the format in the guidelines.
...
- **For batch changes** (multiple skills created or modified in the same session), use the Agent tool to launch one review subagent per skill in a single assistant message so they run concurrently in the foreground. Each Agent call uses `model: "opus"` and sets `run_in_background: false` explicitly; omitting it backgrounds the agent. State the total count explicitly when emitting the calls.
