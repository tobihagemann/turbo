...
Run the `/smoke-test` skill to produce the smoke test plan. Delegate test execution to a subagent using the Agent tool in the foreground (`model: "opus"`, `run_in_background: false`; omitting it backgrounds the agent). Pass the plan and the diff command (`git diff --cached`) to the subagent.
