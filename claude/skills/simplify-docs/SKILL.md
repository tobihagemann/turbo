...
**Markdown files in scope** (READMEs, AGENTS.md, CLAUDE.md, docs/, contributor guides), against every criterion below. This set is empty when the scope holds no markdown.

**The repo's own documentation** (its instruction files, README, and `docs/`) when the scope is a diff, against explanation rot alone: prose the changeset falsified. This agent's prompt carries a summary of what the change did in place of a file list.
