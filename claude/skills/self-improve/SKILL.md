...
6. **Updates to turbo skills** — For each lesson routed to a turbo skill:
   ...
   Once every turbo skill edit is in place and reviewed, use `AskUserQuestion` to ask "These turbo skill improvements could benefit other users. Propose them upstream?" When the user confirms, run `/contribute-turbo` once for all of them.
