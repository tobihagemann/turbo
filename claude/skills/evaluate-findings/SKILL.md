...
Launch a single subagent in the foreground (`model: "opus"`, `run_in_background: false`; omitting it backgrounds the agent). Provide the Medium/Low-confidence findings with their file locations, claims, and initial verdicts. Instruct the subagent to challenge each finding: try to prove it wrong, or confirm it with evidence.
