...
**Iteration 3 or later, if Step 4 of this run made changes**, the hard cap is reached. This replaces the classification gate above for iteration 3 and every iteration after it. Output a summary of what is still changing and whether it is structural or prose-only. Then use `AskUserQuestion` to offer four options: continue for another iteration; "Handoff, then compact", which saves a resume point so the user can compact the session before that iteration; stop here and accept the plan as-is; or escalate to `/consult-oracle` for a different perspective on the remaining issues.

**On "Handoff, then compact"**, run the `/create-handoff` skill, or, when this session already wrote a handoff, edit that file and point its next step at the re-run. Either way, the handoff carries the number of the iteration about to run, the already-adjudicated list this step supplies to it, and the resolved plan path. Then use `TaskUpdate` to set this step's task description to the re-run: run `/refine-plan` on that path as that iteration, reading the already-adjudicated list from the handoff at its path. Leave the task in progress. End the turn there in place of the TaskList call that closes this step, telling the user to run `/compact` and then reply "continue".
