...
- **Complex-project mode** — Multi-subsystem project with architectural decisions. Routes to [`/draft-spec`](skills/draft-spec/SKILL.md) for a guided spec discussion, then [`/refine-spec`](skills/refine-spec/SKILL.md) to iteratively review and revise the spec, then [`/draft-prompt-plan`](skills/draft-prompt-plan/SKILL.md) to decompose the spec into shell plans, then [`/refine-prompt-plan`](skills/refine-prompt-plan/SKILL.md) to review and revise the prompt plan. Halts after the prompt plan is ready; you run [`/pick-next-prompt`](skills/pick-next-prompt/SKILL.md) in fresh sessions to implement each shell.
...
|---|---|---|
| [`/turboplan`](skills/turboplan/SKILL.md) | Universal planning entry: analyzes complexity and routes to small-task (draft → refine → implement), complex-project (draft spec → refine spec → draft prompt plan → refine prompt plan), or shell (fill-in → refine → implement) mode | [`/draft-plan`](skills/draft-plan/SKILL.md), [`/refine-plan`](skills/refine-plan/SKILL.md), [`/implement-plan`](skills/implement-plan/SKILL.md), [`/draft-spec`](skills/draft-spec/SKILL.md), [`/refine-spec`](skills/refine-spec/SKILL.md), [`/draft-prompt-plan`](skills/draft-prompt-plan/SKILL.md), [`/refine-prompt-plan`](skills/refine-prompt-plan/SKILL.md) |
