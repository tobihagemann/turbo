...
4. Add plan Implementation Steps to the active plan
...
Scan the plan's **Implementation Steps** for work types that match available skills. For each unambiguous match, run the skill by reading and following the installed skill instructions. For example, if the plan includes "add a Drizzle migration" and a skill exists whose triggers reference Drizzle migrations, load it. If the plan mentions "run the test suite" but no testing-specific skill trigger matches, do not load a generic testing skill.

If unsure, do not load. Do not load `$code-style` here.

## Step 4: Add Plan Implementation Steps to the Active Plan

Append one plan entry per plan Implementation Step. These entries belong to `$implement`; do not mark them `in_progress` or `completed` in this skill. Codex `update_plan` is flat — there is no parent/child nesting; use clear step text so each entry is self-explanatory in a flat list.
