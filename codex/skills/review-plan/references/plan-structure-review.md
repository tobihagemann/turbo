...
Read project context (AGENTS.md and files mentioned in the plan) to understand the codebase. Review whether the plan holds together and can be executed.
...
- **Test efficacy** — When the plan proposes, keeps, or reshapes a test as a regression net, check that the test can fail when the behavior it guards breaks. Flag an assertion that reads a surface the code under test does not write, a case where a mechanism other than the one under test produces the same observable, or an outcome asserted in a suite that replaces the code producing it with a test double
