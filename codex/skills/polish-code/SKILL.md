...
- **During each iteration:** have the ledger path in context when Steps 3 and 4 run so recorded verdicts are honored. After Step 7's classification, append the iteration number, the round's applied, rejected, and escalated verdicts with reasons, and the classification. For an escalated verdict the user resolved, the reason attributes to the user only what they decided; the details you picked while implementing it stay open to review.
