...
Launch all five agents below with `spawn_agent` / `wait_agent` using inherited model defaults so they run concurrently. Pass the scope from Step 1 to each agent.
