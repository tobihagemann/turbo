...
When the scope contains content that a build or render transform rewrites before it ships — markup compiled to components, template expansion, code generation, translation extraction — build the project before dispatching, in an isolated `git worktree` under `$TMPDIR` when the build writes to tracked files or reaches the shared install through a package-manager wrapper, and name the emitted files as part of the scope every branch receives, so each type judges the emitted artifact rather than the source.
