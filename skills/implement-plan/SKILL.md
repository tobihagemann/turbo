...
After `/finalize` completes, set the plan's frontmatter `status:` to `done`. This closes the loop for `/pick-next-shell`'s dependency resolution, which treats a `.turbo/plans/<dep-slug>.md` with `status: done` as a satisfied dependency. If the plan is the legacy `.turbo/plan.md` without frontmatter, skip this step — it's not tracked by `/pick-next-shell`.
